...
export default function AdminDepositsPage() {
  ...
  async function approve(txId: string) {
    if (!confirm("ยืนยันอนุมัติการเติมเครดิต?")) return;
    const res = await authedAdminFetch("/api/admin/approve-deposit", {
      method: "POST",
  ...
  return (
    <main className="px-4 py-6 max-w-3xl mx-auto">
      <header className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold tracking-tight">คำขอเติมเครดิต</h1>
          <p className="text-sm text-zinc-500">ประเภท deposit เท่านั้น</p>
        </div>
            ...
            <article key={t.id} className="rounded-2xl border border-zinc-800 bg-gradient-to-b from-zinc-950 to-black p-5">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-sm font-semibold truncate">{t.display_name ?? t.user_id}</p>
                  <p className="text-xs text-zinc-500 break-all">{t.user_id}</p>
                  <p className="text-xs text-zinc-500 mt-1">{formatDate(t.created_at)}</p>
                  {t.slip_url ? (
                    <a href={t.slip_url} target="_blank" rel="noreferrer" className="block mt-3">
                      {/* eslint-disable-next-line @next/next/no-img-element -- URL สลิปจาก Supabase ไดนามิก */}
                      <img src={t.slip_url} alt="" className="rounded-xl border border-zinc-800 max-h-48 object-contain" />
                    </a>
                  ) : null}
                </div>
